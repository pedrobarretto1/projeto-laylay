...
O primeiro patcher de produção recusou corretamente ANTES DE ESCREVER:

```text
Tracked working tree ou staging já possui alterações.
produção alterada: NÃO
git add/commit/push: NÃO
```
...
```text
 D HANDOFF_LAYLAY_2026-08-17_ATUALIZADO_21H38_C1B_APLICADA_AUDITADA.md
 D cliente/terminal_2/dashboard.py
 D cliente/terminal_2/sistema_compacto.py
?? HANDOFF_LAYLAY_2026-08-17_ATUALIZADO_22H33_C1B2_PATCHER_PRODUCAO_PRONTO.md
?? aplicar_c1b2_turno155_runtime_real_teste3_6.py
```

`git diff --cached --name-only` ficou vazio.

Conclusão:
- staging estava limpo;
- os dois alvos C1-B2 NÃO estavam modificados;
- havia três deleções tracked preexistentes fora do escopo C1-B2;
- os dois arquivos novos eram untracked e não representam conflito;
- a pré-condição do patcher V1 era ampla demais: exigia o repositório tracked inteiro limpo.

## Correção da política do patcher — sem alterar o candidato

A V2 NÃO muda:
- `ANCHOR_PROD_ANTIGO`;
- `ANCHOR_PROD_NOVO`;
- SHA-256 esperado da produção antes;
- SHA-256 esperado do candidato de produção;
- transformação da regressão permanente;
- conjunto de testes C1-B2/P0/C1-A/R1.1;
- teste runtime detector/orquestrador;
- dívida separada `maximiza opera -> pera`.

A V2 muda SOMENTE o contrato de convivência com alterações tracked preexistentes.

### Novo contrato

Antes de escrever:
1. HEAD e blobs continuam travados;
2. staging precisa estar vazio;
3. os dois alvos C1-B2 precisam estar limpos;
4. coleta o conjunto de alterações tracked preexistentes;
5. gera `git diff --binary` desses caminhos;
6. guarda SHA/conteúdo desse diff.

Depois da escrita e depois dos testes:
1. o conjunto preexistente precisa ser exatamente o mesmo;
2. o `git diff --binary` preexistente precisa ser byte-a-byte idêntico;
3. nenhum novo tracked fora dos dois alvos pode aparecer;
4. staging precisa continuar idêntico ao inicial;
5. os únicos tracked adicionais permitidos são:
   - `mente_laylay/autonomia/roteador_deterministico.py`;
   - `tests/test_regressao_c1b_turno155_maximiza_eliptico.py`.

Em falha:
- rollback escreve SOMENTE os bytes originais dos dois alvos;
- nunca usa `git restore`, `checkout`, `reset`, `add`, `commit` ou `push`;
- valida novamente que o diff preexistente e o staging continuam idênticos.

## Reanálise do patcher pronto

Foi feita segunda revisão integral da V2.

Validações:
- AST/compile: PASS;
- `py_compile`: PASS;
- `--help`: PASS;
- transformação de produção comparada com V1: IDÊNTICA;
- transformação da regressão comparada com V1: IDÊNTICA;
- constantes SHA do candidato: IDÊNTICAS.

### Simulação end-to-end — sucesso

Repo temporário com:
- dois alvos limpos;
- três arquivos tracked deletados antes do patch;
- staging vazio.

Resultado:
```text
✅ C1-B2 PATCH APLICADO E REGRESSÕES VERDES
tracked preexistente preservado: SIM
staging preservado: SIM
```

Após o sucesso:
- três deleções preexistentes continuaram deletadas;
- somente os dois alvos C1-B2 foram acrescentados ao diff;
- staging permaneceu vazio.

### Simulação end-to-end — falha forçada

Foi forçado um pytest vermelho após a escrita.

Resultado:
```text
rollback executado: SIM
bytes originais restaurados: SIM
tracked preexistente preservado: SIM
staging preservado: SIM
```

Após rollback:
- os dois alvos voltaram aos bytes exatos anteriores;
- as três deleções preexistentes permaneceram exatamente iguais;
- nenhum `git restore/reset/add/commit/push` foi usado.

## Artefato atual

`aplicar_c1b2_turno155_runtime_real_teste3_6_v2.py`

SHA-256:
`25828d45fb8d5510d31acee575cf7794ff5f12ae0be3cf73556849ffacf8c695`

Estado:
**PATCHER V2 REANALISADO / PRONTO PARA EXECUÇÃO**
...
```powershell
& C:\Python314\python.exe ".\aplicar_c1b2_turno155_runtime_real_teste3_6_v2.py" --repo .
```

A saída deve listar primeiro os três tracked preexistentes preservados e,
no sucesso, terminar com:

```text
✅ C1-B2 PATCH APLICADO E REGRESSÕES VERDES
produção alterada: SIM — somente escopo C1-B2 permitido
tracked preexistente preservado: SIM
staging preservado: SIM
regressão permanente callback real: SIM
rollback executado: NÃO
git add/commit/push: NÃO
estado: PENDENTE DE TESTE REAL DO CORREDOR 154→155
```

Depois enviar:
- saída completa;
- `diff_patch_c1b2_turno155.diff`;
- `log_patch_c1b2_turno155.txt`;
- `manifest_patch_c1b2_turno155.json`;
- opcionalmente `preexisting_tracked_before.diff` para auditoria adicional.

C1-B2 continua aberto até o corredor real 154→155 confirmar execução.

---

# ESTADO ATUAL — 2026-08-17 22:33 — C1-B2 PATCHER DE PRODUÇÃO PRONTO

## Decisão após auditoria real
Os artefatos reais enviados pelo usuário foram auditados:
- baseline guards/falsificações: 11 PASS;
- baseline detector real: RED causal esperado;
- baseline orquestrador real: RED causal esperado;
- candidato C1-B2: 13 PASS;
- C1-B1: 12 PASS;
- P0: 9 PASS;
- C1-A: 13 PASS;
- R1.1: 44 PASS;
- diff real limitado ao gate de `detectar_janela_contextual()`;
- manifest coerente com HEAD/blobs e produção intacta;
- SHA-256 do diff conferido contra o arquivo real.

Conclusão:
**candidato C1-B2 aprovado para aplicação transacional na produção.**

## Buraco de cobertura permanente confirmado
A regressão atual
`tests/test_regressao_c1b_turno155_maximiza_eliptico.py`
usa `classificar_modalidade_turno(...).depende_contexto` como callback do detector.
Isso difere do runtime real.

O patcher fecha esse seam no MESMO root C1-B2:
- importa `compatibilidade_contexto.texto_depende_de_contexto`;
- usa esse callback no detector da regressão;
- congela:
  - `maximiza` -> referência linguística False;
  - `maximiza ele` -> referência linguística True.

## Escopo exato do patcher
Tracked files permitidos: **2**.

1. `mente_laylay/autonomia/roteador_deterministico.py`
   - mudança de produção já auditada:
     `referencia_linguistica OR (t == "maximiza")`.

2. `tests/test_regressao_c1b_turno155_maximiza_eliptico.py`
   - troca somente o seam falso pelo callback real;
   - acrescenta um guard linguístico;
   - passa de 12 para 13 testes esperados.
...
Dívida separada preservada:
`detectar_janela_explicita("maximiza opera") -> nome_app="pera"`.

## Novo artefato
### `aplicar_c1b2_turno155_runtime_real_teste3_6.py`
- HEAD lock: `f53c9f4ca4165a0bbdecac332b84a89fe993e765`;
- locks dos blobs causais + regressão C1-B;
- exige tracked working tree e staging limpos;
- permite untracked;
- backup byte-a-byte fora do checkout;
- exige SHA-256 baseline da produção;
- exige reprodução do SHA-256 do candidato já auditado;
- aplica por âncoras únicas;
- `py_compile`;
- C1-B/C1-B2: espera 13 PASS;
- P0: espera 9 PASS;
- C1-A: espera 13 PASS;
- R1.1: espera 44 PASS;
- teste runtime temporário: 5 PASS;
- remove o teste temporário;
- garante diff tracked exatamente nos dois arquivos permitidos;
- staging permanece vazio;
- não executa `git add`, `commit` ou `push`;
- rollback automático byte-a-byte em falha;
- gera backup/diff/log/manifest timestampados fora do repo;
- sucesso ainda fica **PENDENTE DE TESTE REAL 154→155**;
- AST: PASS;
- `py_compile` do patcher: PASS;
- `--help`: PASS;
- SHA-256: `bb18c74568fe647b896729bab5523d2b5b92aacbbcd52d309314339828f9b42a`.
...
Só aceitar se terminar com:

```text
✅ C1-B2 PATCH APLICADO E REGRESSÕES VERDES
produção alterada: SIM — somente escopo permitido
regressão permanente callback real: SIM
rollback executado: NÃO
git add/commit/push: NÃO
estado: PENDENTE DE TESTE REAL DO CORREDOR 154→155
...
Depois:
1. enviar a saída completa;
2. enviar `diff`, `log` e `manifest`;
3. auditar a aplicação real;
4. rodar corredor real 154→155/caos;
5. só então fechar C1-B2.

---

# ESTADO ATUAL — 2026-08-17 22:24 — C1-B2 AUDITOR V2 PRONTO

## Resultado do primeiro auditor
O primeiro auditor recusou antes de testar o candidato porque um guard adicional,
fora da causa C1-B2, encontrou:

```text
detectar_janela_explicita("maximiza opera")
-> MAXIMIZE_WINDOW(nome_app="pera")
```

Os outros 11 guards/falsificações selecionados passaram.

## Estudo da recusa — conclusão

### Não é falha do candidato C1-B2
A recusa ocorreu no BASELINE, antes de qualquer aplicação do candidato no espelho.
Logo ela não foi causada pela exceção elíptica proposta.

### É um bug real e separado no detector explícito
No HEAD `f53c9f4ca4165a0bbdecac332b84a89fe993e765`,
`detectar_janela_explicita()` possui o padrão conceitual:

```python
... \s+(?:o|a)?\s*(.+)$
```

O artigo opcional não exige fronteira/espaço. Assim nomes que começam em `o` ou `a`
podem perder a primeira letra:
- `maximiza opera` -> `pera`;
- `maximiza anydesk` -> `nydesk`.

O próprio arquivo já documenta e corrige o mesmo tipo de falha em
`extrair_intencao_abrir_app()`, onde o artigo só é aceito quando termina em espaço.

O orquestrador real chama:
1. `detectar_janela_contextual(...)`;
2. `detectar_janela_explicita(...)`.

Portanto a dívida do detector explícito é real em runtime.

## Separação de raízes
- **C1-B2 atual:** turno 155, `maximiza` SEM alvo nominal; gate do detector contextual.
- **Nova dívida separada:** parsing explícito de nomes iniciados por `o`/`a`;
  exemplo `maximiza opera` -> `pera`.
- A nova dívida NÃO será corrigida junto com C1-B2.
- O guard explícito foi removido do auditor C1-B2 porque fazia uma raiz independente
  bloquear a prova de outra raiz.

## Auditor v2
Novo artefato:
`auditar_candidato_c1b2_turno155_runtime_real_teste3_6_v2.py`

Mudanças em relação ao v1:
- remove somente `test_guard_detector_explicito_continua_resolvendo_alvo_nominal`;
- remove o import temporário agora não usado de `detectar_janela_explicita`;
- adiciona nota de escopo sobre a dívida separada;
- NÃO altera `ANCHOR_ANTIGO`;
- NÃO altera `ANCHOR_NOVO`;
- NÃO altera a regra candidata `acao_janela_eliptica = t == "maximiza"`;
- NÃO altera regressões focadas;
- NÃO escreve na produção.

Validação local do artefato:
- AST/compile: PASS;
- `py_compile`: PASS;
- `--help`: PASS;
- SHA-256: `df0b3f3e97d1a05d7461e45e638ea8df226ec5a434cd0c0fa59849fcf9ec98b4`;
- SHA-256 do diff v1->v2: `8830cf77acc492f8d700be64d09faad5b8ed2698b85c006ecc2a61de04a6b2a5`.

## Próximo passo exato

Executar na raiz da Laylay:

```powershell
& C:\Python314\python.exe ".\auditar_candidato_c1b2_turno155_runtime_real_teste3_6_v2.py" --repo .
```

Esperado:
- baseline guards/falsificações: PASS;
- baseline detector real: RED esperado;
- baseline orquestrador real: RED esperado;
- candidate `py_compile`: PASS;
- candidate C1-B2 runtime real: PASS;
- regressivos C1-B1/P0/C1-A/R1.1: PASS;
- produção real: intacta;
- working tree: preservada.

Só depois de esse candidato ficar verde em espelho devemos auditar diff/log/manifest
antes de qualquer patch de produção.

---

# ESTADO ATUAL — 2026-08-17 22:19 — C1-B2 CANDIDATO EM ESPELHO PRONTO

## Regra de artefatos
Este handoff foi atualizado no mesmo passo da criação do novo artefato C1-B2.

## HEAD soberano
- HEAD esperado: `f53c9f4ca4165a0bbdecac332b84a89fe993e765`
- commit: `teste 3.6`
- produção real ainda não deve ser alterada nesta etapa.

## RED C1-B2 confirmado
O usuário executou o RED e obteve:
- guards PASS;
- detector real `maximiza+app` em RED esperado;
- orquestrador real `maximiza+app` em RED esperado;
...
### Produção — C1-B2
`detectar_janela_contextual()` usa o callback de referência linguística como gate antes
de consultar o último app. No runtime, esse callback termina em
`texto_tem_referencia_contextual()`.

Logo:
- `texto_depende_de_contexto("maximiza") == False` é correto;
- `maximiza` ainda é uma ação elíptica de janela;
- o alvo pode vir do contexto operacional tipado;
- o detector retorna cedo demais e não consulta `ultimo_app_janela`.

### Cobertura — C1-B1
A regressão C1-B1 anterior injetava `classificar_modalidade_turno(...).depende_contexto`
no detector, diferente do callback real. Isso mascarava C1-B2.

## Desenho congelado do candidato
Única mudança proposta em produção, ainda somente no espelho:
...
- `texto_depende_de_contexto("maximiza")` continua `False`;
- `maximiza ele` continua pela referência linguística;
- `maximiza` exige app operacional vivo;
- contexto de site não vira app;
- `não maximiza` não usa a exceção;
- `maximizar` não ganha a exceção;
- `maximize` não ganha a exceção;
- `fecha` e `abre` sem alvo não herdam a regra;
- C1-B1, C1-A, árbitro e orquestrador permanecem intocados.

## Artefato novo
### `auditar_candidato_c1b2_turno155_runtime_real_teste3_6.py`
- propósito: provar/falsificar o candidato em dois espelhos temporários;
- HEAD/blobs travados;
- produção real nunca escrita;
- baseline exige guards verdes e os dois REDs causais;
- candidato altera apenas `roteador_deterministico.py` no espelho;
- usa callback real de referência linguística;
- testa negativos e regressões focadas;
- gera diff/log/manifest fora do checkout;
- verifica working tree real;
- AST: PASS;
- `py_compile`: PASS;
- `--help`: PASS;
- SHA-256: `655a84b86fc04323ad2436a317e598450be2e7513aa23626f3f801f94099bff1`.

## Próximo passo exato
Copiar o auditor para a raiz do repositório e executar:

```powershell
& C:\Python314\python.exe ".\auditar_candidato_c1b2_turno155_runtime_real_teste3_6.py" --repo .
```

Só avançar para patcher de produção se terminar em:

```text
✅ CANDIDATO C1-B2 VERDE EM ESPELHO
produção alterada: NÃO
callback real `maximiza` => referência linguística: NÃO
exceção elíptica exata `maximiza`: SIM
app vivo obrigatório: SIM
working tree preservada: SIM
```

Se qualquer guarda, falsificação ou regressão falhar: não aplicar patch.

---

# HANDOFF INTERNO — PROJETO LAYLAY
Atualizado em: 2026-08-17 21:52 (America/Sao_Paulo)

# ESTADO ATUAL — TESTE 3.6 / C1-B1 APLICADA / C1-B2 RED PRONTO
...
Baseline anterior:
`e06677c4efa652d8225e7979cb4152c63d432730` — `teste 3.5`

## Resultado real 3.6 — corredor 154→159

Novo resultado:
`resultados_testes/roteiro_teste_laylay_caos-20260817-213844-915472/`

Matriz semântica:
- 154: `APP_OPEN` — falhou na avaliação global;
- 155: `SEM_INTENT` — falhou;
- 156: `SEM_INTENT` — não avaliado;
- 157: `SEM_INTENT` — não avaliado;
- 158: `SEM_INTENT` — não avaliado;
- 159: `CLOSE_APP` — passou.

Terminal real 154:
- Opera observado aberto e em foco;
- status `ja_aberto_focado`;
- `executou=False`;
- `confirmado=True`;
- plano `APP_OPEN`.

Isso exercitou diretamente o ramo C1-A que havia sido corrigido.
...
resposta sem contrato operacional
```

Portanto C1-B NÃO está fechada end-to-end.

## Nova separação

### C1-B1 — autoridade da ação
...
Estado:
**corrigida, necessária, regressão permanente verde.**

### C1-B2 — materialização contextual no detector real

Causa encontrada:

`detectar_janela_contextual()` usa:
```text
texto_depende_de_contexto(t)
OU presença de ele/ela/isso
```

Mas o callback real `texto_depende_de_contexto()` é um adaptador de
**referência linguística**, baseado em:
- pronomes;
- reparos;
- ordinais;
- formas referenciais.
...
Assim:
```text
texto_depende_de_contexto("maximiza") == False
```

Isso é coerente para referência linguística, mas insuficiente como proxy para
**ação elíptica que exige alvo contextual**.

Consequência:
- estado possui `ultimo_app_janela=opera`;
- autoridade C1-B1 está correta;
- detector de janela retorna `None` antes de consultar o app;
- orquestrador não publica candidato;
- turno cai na IA;
- caos 3.6 registra `SEM_INTENT`.

Não corrigir globalizando `texto_depende_de_contexto("maximiza")`.
`maximiza` não é uma referência linguística.

Hipótese de correção futura:
o detector de janela deve reconhecer estreitamente a própria forma elíptica
`maximiza` como candidata contextual, e somente então exigir um app vivo.
O árbitro continua sendo a autoridade para efeito.

## RED C1-B2 criado

`red_c1b2_turno155_detector_contextual_runtime_teste3_6.py`

SHA-256:
`ca5252ae997d2c5609c731676ecf0ba664653fe13bc25f8de578838cd3fb6dbf`

Status:
**runner pronto / AST PASS / py_compile PASS / --help PASS**.

HEAD/blobs travados:
- modalidade: `80ddf3ac498cb9cf2cfdbb7d74e0e770d2d9e241`
- roteador determinístico: `6b811913bbe62bf8d399b10ea53ffb4855e7c287`
- orquestrador determinístico: `1ace7364d3ac9ef3530e7cd22607d6573f1c5b86`
- compatibilidade de contexto: `768944f808002d8c24f697c0b2769a31d536eb3e`
- referências de linguagem: `6f1b759fc190228a9f2c4e2c9620c716fe064b53`

### Guards

O runner exige:
1. C1-B1 continua autorizando `maximiza`;
2. `maximiza` NÃO vira referência linguística genérica;
3. `maximiza ele` continua sendo referência;
4. `preparar_entrada_deterministica` chega a `status=ok`;
5. detector funciona quando recebe sinal contextual controlado;
6. sem app vivo não fabrica alvo;
7. contexto site não vira app;
8. `não maximiza` não usa a exceção elíptica.

### REDs

1. detector real + callback real + app Opera:
   espera `MAXIMIZE_WINDOW(opera)`;
2. orquestrador determinístico real + estado Opera:
   espera `MAXIMIZE_WINDOW(opera)`.

Esses REDs cobrem o gate que o RED C1-B1 não cobria.

## Comando

```powershell
& C:\Python314\python.exe ".\red_c1b2_turno155_detector_contextual_runtime_teste3_6.py" --repo .
```

Esperado:
```text
guards ................................ PASS
detector real maximiza+app ............. RED esperado
orquestrador real maximiza+app ......... RED esperado
regressivos focados .................... PASS

PROVA C1-B2: VERMELHO CAUSAL CONFIRMADO
produção alterada: NÃO
working tree preservada: SIM
```
...
Somente se o RED sair exatamente assim:
- montar candidato em espelho;
- preferir mudança estreita em `detectar_janela_contextual`;
- NÃO alterar `texto_depende_de_contexto` global;
- NÃO mexer novamente na autoridade C1-B1;
- manter árbitro intacto;
- testar sem app, site e negação;
- depois patch real;
- depois novo caos.

## Regra fixa de arquivos

Mantida:
**qualquer arquivo Laylay criado exige handoff atualizado na mesma etapa.**

Arquivos criados nesta etapa:
1. `red_c1b2_turno155_detector_contextual_runtime_teste3_6.py`
2. `HANDOFF_LAYLAY_2026-08-17_ATUALIZADO_21H52_TESTE_3_6_C1B2_RED_PRONTO.md`

---

# HISTÓRICO ANTERIOR PRESERVADO

Quando houver conflito, prevalece o estado 21:52 acima.

# HANDOFF INTERNO — PROJETO LAYLAY
Atualizado em: 2026-08-17 21:38 (America/Sao_Paulo)

# ESTADO ATUAL — C1-B APLICADA E AUDITADA / AGUARDANDO TESTE REAL

ESTA SEÇÃO SUPERA estados/próximos passos anteriores em caso de conflito.

## Baseline do patch

HEAD travado durante a aplicação:

`e06677c4efa652d8225e7979cb4152c63d432730` — `teste 3.5`

O patcher não fez commit/push.
...
baseline RED autoridade ............ RED esperado
baseline RED barreira .............. RED esperado
baseline RED árbitro ............... RED esperado
candidate regressão C1-B ........... PASS
candidate regressivos focados ...... PASS
produção escrita ................... SIM
regressão permanente (12 testes) ... PASS
regressivos focados reais .......... PASS
roteador_deterministico.py ......... INTACTO
arbitro_turno.py ................... INTACTO
working tree não relacionado ....... PRESERVADO

PATCH C1-B APLICADO E VALIDADO
```
...
## Artefatos recebidos e auditados

### Manifest

`manifest_c1b_turno155_maximiza_eliptico.json`

SHA-256 observado:

`3d7213afcca1e646585fe1c6b1ff1fc660b823678b3569117f0129b3b8438e05`

### Diff

`c1b_turno155_maximiza_eliptico.diff`

SHA-256 observado:

`da698b1b7be541f104c5ff7b5cce9791037c73f641be4fa2a2dece37b23ae792`
...
**SHA do diff recebido coincide exatamente com o manifest.**

---

## Escopo auditado

`patched_production` do manifest:
...
Roteador:
- before SHA = `935da6470e50f0d16602357acb00779e41e32ae23af5a8b0384d611cfd99470e`
- after SHA  = `935da6470e50f0d16602357acb00779e41e32ae23af5a8b0384d611cfd99470e`
- estado: **INTACTO**

Árbitro:
- before SHA = `413dbfe0051fc6673abc73c0719953ca68ba374377e86028ff9ac84884a876cc`
- after SHA  = `413dbfe0051fc6673abc73c0719953ca68ba374377e86028ff9ac84884a876cc`
- estado: **INTACTO**

Modalidade:
- before SHA = `09fff34ad655bfb5b77943c47597ce3448d1460630974693ed1ecde99860c9eb`
- after SHA  = `7237ed6faeafec5f39bd4214dfd41ff2fa6da50828471163d0406875429b9b6f`

Regressão:
- SHA = `0ef65133861dc689e50d9d366067646b9fc820d6a39099f1d3f5711e9ec8da54`

---

## Diff de produção — auditoria semântica

A única mudança de comportamento em `modalidade_turno.py` é:
...
ALVO
    → continua ausente na superfície
    → deve vir de contexto tipado válido
```

Nenhum outro verbo sem alvo foi globalizado.

Whitelist registrada no manifest:
...
- 3 contratos causais:
  ...
  - árbitro;
- 1 guard adicional:
  - `maximiza` autorizado sem referência app
    NÃO fabrica alvo nem decisão.

Regressões históricas reais após escrita:
- P0 autorização/modalidade: PASS;
- C1-A buffer operacional: PASS;
- R1.1 navegador/autoridade/cadeia: PASS.

---

## Estado técnico da C1-B

```text
estudo profundo ............... FEITO
causa raiz .................... PROVADA
RED causal .................... PROVADO
higiene RED ................... PROVADA
candidato espelho ............. VERDE
diff candidato ................ ESTREITO
patch real .................... APLICADO
regressão permanente .......... VERDE
P0 / C1-A / R1.1 .............. VERDES
roteador / árbitro ............ INTACTOS
diff + manifest ............... AUDITADOS
teste real 154→159 ............ PENDENTE
```

C1-B ainda NÃO deve ser chamada de encerrada end-to-end antes do teste real.

---

## Próximo passo

Executar novo corredor/caos real contendo:
...
Turno 155 deve agora publicar algo equivalente a:

```text
MAXIMIZE_WINDOW
nome_app=opera
referência contextual válida
```

e o executor deve confirmar o resultado real.

Não usar falhas de 156/157/158 para reabrir C1-B automaticamente.

Separação mantida:

```text
C1-B = maximiza
C1-C = esquerda / direita
C1-D = agora a calculadora
```

Depois do teste real:
- se 155 executar e confirmar corretamente, fechar C1-B;
- se 155 ainda falhar, estudar o novo ponto exato do pipeline;
- não fazer mega-patch.
...
Nesta etapa foi criado apenas este novo handoff.

---

# HISTÓRICO ANTERIOR PRESERVADO

Quando houver conflito, prevalece o estado 21:38 acima.

# HANDOFF INTERNO — PROJETO LAYLAY
Atualizado em: 2026-08-17 21:34 (America/Sao_Paulo)

# ESTADO ATUAL — C1-B CANDIDATO VERDE / PATCH DE PRODUÇÃO PRONTO
...
Rechecado antes da geração do patcher: nenhum commit novo.

## C1-B — candidato em espelho APROVADO

Auditor:
`auditar_candidato_c1b_turno155_maximiza_eliptico_teste3_5.py`
...
baseline RED barreira .............. RED esperado
baseline RED árbitro ............... RED esperado
candidate C1-B completo ............ PASS
regressivos focados ................ PASS
modalidade_turno.py candidate ...... ALTERADO NO ESPELHO
roteador_deterministico.py ......... INTACTO
arbitro_turno.py ................... INTACTO

CANDIDATO C1-B VERDE EM ESPELHO
produção alterada: NÃO
working tree preservada: SIM
```

O log detalhado confirmou:
- candidate C1-B: 12 PASS;
- P0: 9 PASS;
- C1-A: 13 PASS;
- R1.1: 44 PASS.

## Política C1-B congelada

Somente a forma normalizada exata:

`maximiza`

ganha autoridade para a AÇÃO quando não há alvo superficial.

Continuam verdadeiros:
- `requer_esclarecimento=True`;
- `depende_contexto=True`;
- contexto deve fornecer um app tipado válido;
- sem referência app, não há candidato/decisão;
- referência de domínio errado é rejeitada;
- outros verbos sem alvo continuam não autorizados.

Não ampliar nesta etapa para:
- `maximize`;
- `maximizar`;
- `abre`;
- `fecha`;
...
Único fonte alterável:
`mente_laylay/cognicao/modalidade_turno.py`

Devem permanecer byte-idênticos:
- `mente_laylay/autonomia/roteador_deterministico.py`
- `mente_laylay/cognicao/arbitro_turno.py`

## Regressão permanente criada

`test_regressao_c1b_turno155_maximiza_eliptico.py`

SHA-256:
`0ef65133861dc689e50d9d366067646b9fc820d6a39099f1d3f5711e9ec8da54`

Estado:
**AST PASS / 12 testes permanentes**.

Cobertura:
- 8 guards originais;
- 3 contratos causais C1-B;
- 1 guard adicional: `maximiza` autorizado sem referência NÃO fabrica alvo nem decisão.

## Patcher de produção criado

`patch_c1b_turno155_maximiza_eliptico_teste3_5.py`

SHA-256:
`a58617a0c2fbc73c6d29a662c97e38e274b7ceae9dea48fdd345d7c85aab7c2c`

Estado:
**AST PASS / py_compile PASS / --help PASS**.

O patcher:
1. trava HEAD + blobs modalidade/roteador/árbitro;
2. exige fontes causais limpas;
3. exige regressão nova ausente;
4. preserva status de arquivos não relacionados;
5. repete baseline guards + 3 REDs em espelho;
6. aplica candidato no espelho;
7. roda regressão permanente no espelho;
8. roda P0 + C1-A + R1.1 no espelho;
9. exige roteador e árbitro byte-idênticos;
10. revalida HEAD/blobs/status antes da escrita;
11. escreve somente modalidade + regressão;
12. preserva BOM e CRLF/LF da modalidade real;
13. roda py_compile real;
14. roda regressão permanente de 12 testes;
15. roda P0 + C1-A + R1.1 novamente no worktree real;
16. exige roteador e árbitro intactos;
17. exige whitelist única `t == "maximiza"`;
18. gera diff/log/manifest fora do repo;
19. em falha após escrita, restaura modalidade byte-a-byte e remove só a regressão nova;
20. nunca executa add/commit/push/reset/checkout/restore.

## Comando

```powershell
& C:\Python314\python.exe ".\patch_c1b_turno155_maximiza_eliptico_teste3_5.py" --repo .
```
...
- depois testar corredor real 154→159;
- o turno-chave C1-B é 155 `maximiza`;
- manter 156/158 (`esquerda/direita`) como C1-C separada;
- manter 157 (`agora a calculadora`) como C1-D separada.
...
Arquivos criados nesta etapa:
1. `test_regressao_c1b_turno155_maximiza_eliptico.py`
2. `patch_c1b_turno155_maximiza_eliptico_teste3_5.py`
3. `HANDOFF_LAYLAY_2026-08-17_ATUALIZADO_21H34_C1B_PATCH_PRONTO.md`

---

# HISTÓRICO ANTERIOR PRESERVADO

Quando houver conflito, prevalece o estado 21:34 acima.

# HANDOFF INTERNO — PROJETO LAYLAY
Atualizado em: 2026-08-17 21:25 (America/Sao_Paulo)

# ESTADO ATUAL — C1-B RED FECHADO / CANDIDATO EM ESPELHO PRONTO
...
Rechecado antes de montar o candidato: nenhum commit novo.

## C1-B — RED V2 FECHADO

Runner:

`red_c1b_turno155_maximiza_eliptico_teste3_5_v2.py`

Resultado real informado pelo usuário:

```text
guards: PASS
RED autoridade: esperado
RED barreira: esperado
RED árbitro: esperado
produção alterada: NÃO
git add/commit/push: NÃO
```

Não houve traceback nem divergência final de working tree.

Regressões durante a prova:
- P0 autorização/modalidade: 9 PASS;
- C1-A buffer operacional: 13 PASS;
- R1.1 navegador/autoridade/cadeia: 44 PASS.

Conclusão:
**C1-B tem causa causal e higiene de runner comprovadas.**

## Causa congelada

Baseline de `maximiza`:
...
O detector contextual já produz:
...
A barreira e o árbitro rejeitam o efeito porque
`autoriza_execucao=False`.

Portanto:
- ação está explícita na fala atual;
- alvo não está explícito;
- contexto deve fornecer somente o alvo;
- contexto não pode criar autoridade.
...
`5e723632fd6260f5a2077e757f22d116f8b1cdbd8eb45ab2f06f9873b374e76b`

Estado:
**auditor pronto / AST PASS / py_compile PASS / --help PASS**.

### Escopo candidato

ÚNICO fonte candidato:

`mente_laylay/cognicao/modalidade_turno.py`

Arquivos que DEVEM permanecer intactos:

- `mente_laylay/autonomia/roteador_deterministico.py`
- `mente_laylay/cognicao/arbitro_turno.py`

Mudança conceitual exata:

```text
se:
    alvo_ausente=True
    texto normalizado == "maximiza"

então:
    autoriza a AÇÃO de maximizar
    mantém requer_esclarecimento=True
    mantém depende_contexto=True
```

Nenhum outro verbo sem alvo ganha essa exceção.

A whitelist é deliberadamente uma única forma:

`maximiza`

Nesta etapa NÃO ampliar para:
- maximize;
- maximizar;
- abre;
- fecha;
...
## Guard novo do candidato

Além dos 8 guards e 3 REDs já provados, o espelho candidato exige:

```text
"maximiza"
+ sem referência app
→ detector não fabrica alvo
→ árbitro sem candidato não executa nada
```

Isso congela a separação:

```text
AUTORIDADE DA AÇÃO = fala atual
ALVO = somente referência tipada válida
```

## O auditor

Ele:
1. trava HEAD + blobs de modalidade/roteador/árbitro;
2. exige os 3 fontes causais sem edição local;
3. cria baseline/candidate por `git archive HEAD`;
4. roda os 8 guards no baseline;
5. exige os 3 REDs causais;
6. aplica candidato somente no espelho;
7. roda py_compile;
8. exige teste C1-B completo verde;
9. roda P0 + C1-A + R1.1 no candidato;
10. exige roteador e árbitro byte-idênticos;
11. gera diff somente de `modalidade_turno.py`;
12. audita whitelist exata `t == "maximiza"`;
13. grava diff/log/manifest fora do repo;
14. exige working tree real idêntico no fim;
15. não executa add/commit/push/reset/checkout/restore.
...
```powershell
& C:\Python314\python.exe ".\auditar_candidato_c1b_turno155_maximiza_eliptico_teste3_5.py" --repo .
```
...
Se green:
- auditar diff + manifest;
- só então criar patcher de produção + regressão permanente;
- ainda não misturar C1-C (`esquerda/direita`) nem C1-D (`agora a calculadora`).
...
Arquivos criados nesta etapa:
1. `auditar_candidato_c1b_turno155_maximiza_eliptico_teste3_5.py`
2. `HANDOFF_LAYLAY_2026-08-17_ATUALIZADO_21H25_C1B_RED_FECHADO_CANDIDATO_PRONTO.md`

---

# HISTÓRICO ANTERIOR PRESERVADO

Quando houver conflito, prevalece o estado 21:25 acima.

# HANDOFF INTERNO — PROJETO LAYLAY
Atualizado em: 2026-08-17 21:17 (America/Sao_Paulo)

# ESTADO ATUAL — C1-B RED CAUSAL PROVADO / HIGIENE DE STATUS EM DIAGNÓSTICO
...
## C1-B — resultado real do RED V1

Runner:
`red_c1b_turno155_maximiza_eliptico_teste3_5.py`

SHA-256:
`3516316e1e4ba92a5439a68df98f4e4a2ee845982f0822d2f94853ed42bb99c0`

A execução real do usuário provou:

- 8 guards C1-B: PASS;
- RED autoridade `maximiza`: falha causal esperada;
- RED barreira P0: falha causal esperada;
- RED árbitro com referência app tipada: falha causal esperada;
- `tests/test_p0_autorizacao_modalidade.py`: 9 PASS;
- `tests/test_regressao_c1_turno159_buffer_operacional.py`: 13 PASS;
- `tests/test_regressao_r1_1_autoridade_navegador_cadeia.py`: 44 PASS.
...
O detector contextual, no mesmo baseline, já produz:

```text
MAXIMIZE_WINDOW(nome_app="opera")
```

quando recebe um app vivo.

O árbitro rejeita esse candidato especificamente porque a autoridade
congelada do turno é `False`.

Portanto:
**a raiz causal da C1-B está provada.**

## Por que o runner terminou com erro

Depois de toda a prova causal e das regressões verdes, o V1 recusou a higiene:

```text
Working tree mudou após o RED.
```

O V1 removeu o teste temporário, mas só comparava os bytes brutos de
`git status --porcelain -z`; ele NÃO mostrava qual caminho mudou.

Importante:
o `.gitignore` do HEAD 3.5 já ignora:
- `.pytest_cache/`
- `__pycache__/`
- `*.pyc`

Logo não devemos assumir que a divergência foi cache Python.

Também não devemos ignorar o status nem apagar arquivos desconhecidos por
reflexo.
...
SHA-256:
`d1beff25fcd7cadcf1ac857122d77d6b05864159927b4507f0b327e507c96972`

Estado:
**self-check PASS / produção não alterada pelo arquivo**.

A V2 mantém exatamente:
- mesmos HEAD/blobs;
- mesmos 8 guards;
- mesmos 3 REDs;
- mesmas regressões focadas;
- mesmo cleanup do teste temporário;
- mesma comparação fail-closed de working tree.

A única mudança funcional do runner é diagnóstico:
se o status divergir, imprime:

```text
STATUS BEFORE
STATUS AFTER
ADDED/CHANGED DURANTE O RUNNER
REMOVED/RESOLVIDO DURANTE O RUNNER
```

Ela NÃO:
- ignora diferenças;
- apaga arquivos desconhecidos;
- faz restore/reset/checkout;
- modifica produção para resolver a higiene.
...
```powershell
& C:\Python314\python.exe ".\red_c1b_turno155_maximiza_eliptico_teste3_5_v2.py" --repo .
```

Se o status voltar igual, a prova termina green de higiene.

Se divergir novamente, enviar a saída final contendo
`ADDED/CHANGED DURANTE O RUNNER` e `REMOVED/RESOLVIDO DURANTE O RUNNER`.

Só depois identificar a origem real da diferença.

## C1-B — política de engenharia continua

Não criar candidato de produção ainda enquanto a higiene do runner não estiver
explicada.

Depois:
RED causal + higiene fechada
→ candidato em espelho
→ regressões
→ diff auditado
→ patcher
→ caos real.
...
Mantida:
**qualquer arquivo criado para Laylay exige handoff atualizado na mesma etapa.**

Arquivos criados nesta etapa:
1. `red_c1b_turno155_maximiza_eliptico_teste3_5_v2.py`
2. `HANDOFF_LAYLAY_2026-08-17_ATUALIZADO_21H17_C1B_RED_PROVADO_STATUS_DIAGNOSTICO.md`

---

# HISTÓRICO ANTERIOR PRESERVADO

Quando houver conflito, prevalece o estado 21:17 acima.

# HANDOFF INTERNO — PROJETO LAYLAY
Atualizado em: 2026-08-17 21:06 (America/Sao_Paulo)

# ESTADO ATUAL — C1-B ESTUDADA / RED PRONTO / PRODUÇÃO INTACTA

ESTA SEÇÃO SUPERA estados/próximos passos anteriores em caso de conflito.

## HEAD soberano confirmado

`e06677c4efa652d8225e7979cb4152c63d432730`

Mensagem: `teste 3.5`

Blobs causais travados:
- `mente_laylay/cognicao/modalidade_turno.py`
  - `e322b7ab2513639db19b0c5a405f62dd4d0dba7b`
- `mente_laylay/autonomia/roteador_deterministico.py`
  - `6b811913bbe62bf8d399b10ea53ffb4855e7c287`
- `mente_laylay/cognicao/arbitro_turno.py`
...
Continua **FECHADA**. Nada desta etapa altera a política
`APP_OPEN + ja_aberto_focado` nem seus módulos/regressões.

---

# C1-B — TURNO 155 `maximiza`
...
```text
154 Abre o Opera.  -> APP_OPEN opera confirmado
155 maximiza       -> sem intent
...
159 fecha ela      -> CLOSE_APP opera confirmado
```

O 159 correto mostra que Opera permaneceu recuperável. Logo o 155 não é perda
do buffer C1-A.

## Prova causal no HEAD 3.5

### 1. Modalidade

`modalidade_turno.py` já reconhece `maximiza` como imperativo direto.
...
A ação foi reconhecida; o alvo superficial ausente está apagando a autoridade
da própria ação.

### 2. Detector contextual

`roteador_deterministico.py::detectar_janela_contextual(...)` já lê
`ultimo_app_janela` e, para `maximiza/maximizar`, materializa:

```python
{"intent": "MAXIMIZE_WINDOW", "params": {"nome_app": ultimo_app}}
```

Com Opera vivo, o alvo já é resolvível sem IA.

### 3. Árbitro

`arbitro_turno.py` rejeita candidato mutante contextual quando a autoridade
congelada do turno é False. Também protege incompatibilidade de domínio.

Cadeia causal:

```text
"maximiza"
→ ação explícita reconhecida
→ alvo superficial ausente
→ autoriza_execucao=False
→ detector consegue resolver Opera
→ árbitro rejeita por autoridade False
→ sem MAXIMIZE_WINDOW
```

Raiz C1-B:
**autoridade da ação e presença superficial do alvo estão acopladas.**

Contrato desejado:

```text
AUTORIDADE DA AÇÃO <- fala atual
ALVO               <- referência app tipada
...
Status:
`RED runner pronto / produção não alterada`

Validação desta etapa:
- AST runner: PASS;
- compile runner: PASS;
- AST pytest embutido: PASS;
- compile pytest embutido: PASS;
- execução real `--self-check-only`: PASS;
- patch de produção: NÃO;
- commit/push: NÃO.

O ambiente desta conversa não consegue clonar GitHub pelo terminal
(`Could not resolve host: github.com`). Portanto o pytest contra checkout real
não foi fingido como executado.

O runner exige o checkout real e:
- HEAD exato 3.5;
- blobs causais exatos;
- fontes causais sem edição local;
- teste temporário removido em `finally`;
- `git status` restaurado ao estado inicial;
- nenhum Git mutante.

## Guards

1. `maximiza` continua `acao_explicita=True`,
   `depende_contexto=True`, `requer_esclarecimento=True`.
2. Detector + Opera vivo -> `MAXIMIZE_WINDOW(opera)`.
3. Sem app -> não fabrica alvo.
4. `ele/ela` sem verbo -> não fabrica maximização.
5. `Maximiza a Calculadora.` e `Maximiza ele.` continuam autorizados.
6. Negação, hipótese, pergunta e metalinguagem continuam bloqueadas.
7. `abre/fecha/apaga/remove` de uma palavra NÃO ganham autoridade por tabela.
8. Referência de domínio `site` não pode alimentar candidato `app`.

## REDs esperados no baseline

A. `C1B_RED_AUTORIDADE_MAXIMIZA_PURO`
- deseja autoridade para a ação explícita;
- preserva alvo pendente.

B. `C1B_RED_BARREIRA_MAXIMIZA_PURO`
- deseja que a barreira respeite a autoridade da ação.

C. `C1B_RED_ARBITRO_MAXIMIZA_APP`
- usa detector real + referência tipada app;
- deseja decisão `MAXIMIZE_WINDOW(opera)`.

Os três observam a mesma raiz em níveis diferentes:
classificador -> barreira -> árbitro.

---

## Não fazer agora

- não remover `alvo_ausente` globalmente;
- não autorizar todos os verbos de uma palavra;
- não mexer em C1-A;
- não alterar detector de janela que já resolve Opera;
- não relaxar árbitro/R1.1;
- não misturar `esquerda`, `agora a calculadora`, `direita`;
- não criar patch antes do RED real.

---

## Próximo passo

Rodar no clone local, HEAD `e06677c4...`:

```powershell
python "red_c1b_turno155_maximiza_eliptico_teste3_5.py" --repo "CAMINHO\PARA\projeto-laylay"
```

Só aceitar antes do candidato:
- guards PASS;
- 3 REDs esperados;
- regressões focadas PASS;
- produção intacta;
- working tree preservada.

Depois disso:
RED comprovado -> candidato estreito -> espelho -> regressões -> auditoria ->
patch seguro.

---

## Regra fixa arquivos/handoff

**Sempre que qualquer arquivo Laylay for criado, atualizar o handoff na mesma etapa.**

Nesta etapa foram criados juntos:
1. `red_c1b_turno155_maximiza_eliptico_teste3_5.py` — RED runner C1-B;
2. `HANDOFF_LAYLAY_2026-08-17_ATUALIZADO_21H06_C1B_RED_PRONTO.md` — handoff soberano.

Nenhum outro artefato Laylay foi criado nesta etapa.

---

# HISTÓRICO ANTERIOR PRESERVADO

Quando houver conflito, prevalece o estado 21:06 acima.

# HANDOFF INTERNO — PROJETO LAYLAY
Atualizado em: 2026-08-17 20:40 (America/Sao_Paulo)

# ESTADO ATUAL — TESTE 3.5 / C1-A FECHADA / C1-B PRÓXIMA

ESTA SEÇÃO SUPERA estados/próximos passos anteriores em caso de conflito.

## HEAD atual no GitHub
...
**FECHADA no escopo técnico aprovado.**

Causa:
a memória/referência operacional usava semântica antiga de mutação
(`executou=True`) em pontos onde o contrato causal moderno distingue:
...
Esse caso significa que nenhuma nova mutação foi necessária, mas o app está
observado e em foco, portanto continua sendo um referente operacional válido.
...
Produção modificada somente pelo patcher:
...
foi adicionado como estado já satisfeito referenciável.

Não houve globalização de `confirmado=True`.

`resultado_acao.py` permaneceu byte-intacto segundo o manifest.

Regressões focadas passaram tanto no candidato quanto após escrita real:
- `test_adaptador_resultado.py`
- `test_p0_autorizacao_modalidade.py`
- `test_regressao_m1_turno149_cadeia_midia_playlist.py`
- `test_regressao_m1_turno149_wiring_playlist_contexto.py`
- `test_regressao_patch20_r1_r2.py`
- `test_regressao_r1_1_autoridade_navegador_cadeia.py`

Regressão C1 real:
PASS.

Working tree não relacionado:
preservado pelo patcher.

### Integridade dos dois artefatos enviados
...
`85a35019845bdaf5c7949a840ecd9c7767a58a90868347ee8f8920a884ec60af`

Esse SHA coincide exatamente com `diff_sha256` do manifest.

Manifest recebido SHA-256:

`05dbbcac5e7d90ab103c4d1a8de074f99d129db0acdb7622696e8fe8f0292cda`

Regressão permanente SHA-256 registrado no manifest:

`cb2990087c03bc015dfdafe8b5e9bda67a044adfd837cce4ad43f0e69aa27e01`

---

## Auditoria do diff C1

Mudança em `contexto_compartilhado.py`:
- cria `_RESULTADOS_JA_SATISFEITOS_REFERENCIAVEIS`;
- única entrada: `("APP_OPEN", "ja_aberto_focado")`;
- cria `resultado_ja_satisfeito_referenciavel(...)`;
- cria `contrato_confirma_referencia_operacional(...)`;
- `_resultado_pode_promover_referencia(...)` preserva `executou=True`
  e adiciona somente o novo caminho tipado.

Mudança em `contexto_imediato.py`:
- importa `contrato_confirma_referencia_operacional`;
- `_dominio_contrato_referencia(...)` passa a usar o contrato central;
- bridge de `fecha ela` passa a usar o mesmo contrato central.

Invariante preservado:

```text
REFERÊNCIA PODE FORNECER ALVO
REFERÊNCIA NÃO PODE FORNECER AUTORIDADE
```

O verbo/ato atual continua sendo necessário para materializar efeito.
...
`resultados_testes/roteiro_teste_laylay_caos-20260817-202112-307411/`
...
- alertas: 5;
- não avaliados: 214;
- taxa semântica: 49.06%;
- comandos observados: 114;
- confirmações indeterminadas: 10;
- p50: 1.906 s;
- p95: 12.555 s;
- máxima: 37.122 s;
- média: 3.252 s.

Não usar a taxa global para julgar C1 isoladamente:
o caos contém vários domínios e falhas independentes.

### Corredor 154 → 159 no 3.5
...
IMPORTANTE:
nesta execução o Opera não estava em foco antes da ação.
Portanto o turno 154 NÃO exercitou o ramo causal
...
Resultado:
- sem intent;
- falhou semanticamente;
- esperado `MAXIMIZE_WINDOW`.
...
Conclusão:
o corredor real agora mantém Opera como referente até o 159 e fecha o alvo
correto, apesar dos turnos intermediários 155–158 não produzirem ações.

Por rigor causal:
- isso confirma o fechamento/contexto real do 159;
- NÃO é uma prova end-to-end do ramo no-op `ja_aberto_focado` no turno 154,
  porque esse ramo não ocorreu nesta execução;
- o ramo no-op está fechado por RED→green + regressão permanente + patch real.

---

## C1-B — PRÓXIMA RAIZ

Próximo sintoma isolado:

Turno 155:
`maximiza`

Esperado:
`MAXIMIZE_WINDOW` usando o app operacional recente.

Observado:
sem intent.

Esse problema agora é claramente separável de C1-A:
- o referente Opera está vivo;
- `fecha ela` no 159 consegue recuperá-lo;
- o que falta no 155 é a gramática/continuidade de comando curto de janela.

Hipótese a estudar, NÃO patchar ainda:
um comando curto de janela como `maximiza` precisa materializar
`MAXIMIZE_WINDOW` usando uma referência app válida, sem aumentar autoridade
para frases narrativas ou incompletas.

Depois do 155, estudar separadamente:
- 156 `esquerda`;
- 157 `agora a calculadora`;
- 158 `direita`.
...
## Observação de higiene do commit 3.5

O commit `teste 3.5` NÃO é scope-isolado apenas à C1.

Além da C1 e da regressão, ele também:
- adiciona este handoff anterior;
- remove vários resultados antigos em `resultados_testes/`;
- inclui/renomeia o novo resultado de caos 3.5;
- modifica um `terminal.log` do caos 3.4.

Isso não invalida a C1, porque o diff/manifest do patcher é isolado e foi
auditado separadamente.

Mas para auditoria Git futura:
**não usar o commit 3.5 inteiro como se fosse o diff puro da C1.**
O diff puro canônico da C1 é o arquivo
`c1_turno159_buffer_operacional.diff`.
...
Começar C1-B pelo turno 155 `maximiza`.

Antes de qualquer patch:
1. reler HEAD `e06677c...`;
2. rastrear gramática/detector/roteador para `maximiza`;
3. comparar com casos verdes:
   - turno 112 `Maximiza ele.` → PASS;
   - turno 143 `Maximiza a Calculadora...` → PASS;
4. descobrir por que a forma elíptica sem pronome não chega ao mesmo corredor;
5. provar RED estreito;
6. preservar autoridade e domínio tipado.

---

## Regra fixa de arquivos/handoff

Mantida:

**sempre que qualquer arquivo Laylay for criado, atualizar o handoff na mesma etapa.**

Nesta etapa foi criado somente este novo handoff.

---

# HISTÓRICO ANTERIOR PRESERVADO

Quando houver conflito, prevalece o estado 20:40 acima.
