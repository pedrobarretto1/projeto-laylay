...
from cliente.terminal_2.sistema_compacto import (
    CardSistemaCompacto,
    JANELA_HISTORICO_SISTEMA,
    posicoes_historico_sistema,
)
...
class GraficoMetricaSistema(QWidget):
    ...
    def paintEvent(self, event) -> None:  # noqa: N802 - contrato Qt
        ...
        if self._tom in {"ram", "vram"}:
            passo_barra = area.width() / JANELA_HISTORICO_SISTEMA
            ...
            posicoes = posicoes_historico_sistema(len(valores), barras=True)
        ...
        pontos = [
            ...
            for posicao, valor in zip(posicoes_historico_sistema(len(valores)), valores)
        ]
        if len(pontos) == 1:
            painter.setPen(Qt.NoPen)
            painter.setBrush(cor)
