    ...
    *,
    ...
    if "bom dia" in texto_lower:
        if contexto_descanso and periodo in {"madrugada", "noite"}:
            return escolher_fala([
                "Bom dia. Mas esse corpo aí tá pedindo descanso, não cumprimento.",
                "Bom dia meio torto. Parece que teu corpo ainda tá em modo descanso.",
                "Bom dia, mas teu contexto tá pedindo cama, não café.",
            ])
        if contexto_inicio_dia or periodo == "manha":
            return escolher_fala([
                "Bom dia. Bora aproveitar a manhã.",
                "Bom dia. Hora de fazer o dia acontecer.",
                "Bom dia. Vamos começar essa manhã direito.",
            ])
        if periodo == "madrugada":
            return escolher_fala([
                "Boa madrugada. Ainda tá cedo demais até pra fingir que é dia.",
                "Boa madrugada. O relógio tá claramente fora de hora pra bom dia.",
                "Madrugada. Isso aí ainda não virou manhã.",
            ])
        if periodo == "tarde":
            return escolher_fala([
                "Boa tarde. Meio atrasado, mas valeu a intenção.",
                "Boa tarde. Chegou com um pequeno atraso, mas chegou.",
                "Boa tarde. Essa saudação veio no timing caprichado demais.",
            ])
        return escolher_fala([
            "Boa noite. Esse bom dia veio meio perdido, mas eu aceito.",
            "Boa noite. Esse bom dia tropeçou no relógio.",
    ...
    if "boa tarde" in texto_lower:
        if contexto_descanso and periodo == "madrugada":
            return escolher_fala([
                "Boa madrugada. Essa tarde aí tá sonhando alto.",
                "Madrugada. Essa tarde ainda não foi autorizada.",
                "Tá de madrugada, mas a saudação veio de tarde.",
            ])
        if periodo == "manha":
            return escolher_fala([
                "Bom dia ainda. A tarde tá adiantada demais.",
                "Ainda é manhã. Essa tarde chegou cedo demais.",
                "Bom dia, porque a tarde ainda nem acordou.",
            ])
        if contexto_inicio_dia:
            return escolher_fala([
                "Ainda tá com cara de começo de dia. Tarde nenhuma decidiu chegar de verdade.",
                "O dia ainda tá começando, então essa tarde tá meio fictícia.",
                "Isso aí ainda tá com energia de manhã, não de tarde.",
            ])
        if periodo == "noite":
            return escolher_fala([
                "Boa noite. Essa tarde já foi embora faz tempo.",
                "Boa noite. A tarde já encerrou o expediente.",
                "Noite. A tarde já foi dormir.",
            ])
        return escolher_fala([
            "Boa tarde.",
    ...
    if "boa noite" in texto_lower:
        if contexto_descanso:
            return escolher_fala([
                "Boa noite. Acho que o contexto já tá me dizendo pra baixar o ritmo.",
                "Boa noite. O contexto já pediu modo baixo.",
                "Noite. Vou reduzir o volume do papo.",
            ])
        if periodo == "manha":
            return escolher_fala([
                "Bom dia. Essa boa noite tá bem adiantada.",
                "Bom dia. A boa noite veio cedo demais.",
                "Isso ainda é manhã. A noite tá chutando a porta cedo.",
            ])
        if periodo == "tarde":
            return escolher_fala([
                "Ainda é tarde. A noite tá chegando, mas não chegou.",
                "Tarde ainda. A noite tá só ensaiando.",
                "A noite tá vindo, mas ainda não estacionou.",
            ])
        if periodo == "madrugada":
            return escolher_fala([
                "Boa madrugada. Esse boa noite já virou plantão.",
                "Boa madrugada. O boa noite já entrou em turno extra.",
                "Madrugada. Esse cumprimento já tá fazendo hora extra.",
            ])
        return escolher_fala([
            "Boa noite.",
