...
def inferir_tipo_foco_vivo(
    ...
    *,
    ...
    if intent in {"WEATHER"} or hab in {"clima", "tempo"}:
        return "clima"
    if intent in {"SCREEN_CAPTURE", "VISION_QUERY", "GAME_VISION"} or hab in {"visao", "visão", "visao_jogo"}:
        return "visao"
    ...
    if intent in {"PLAYLIST_CREATE", "PLAYLIST_PLAY", "PLAYLIST_ADD", "PLAYLIST_LIST", "PLAYLIST_MOVE", "MUSIC_SEARCH", "MEDIA_CONTROL", "MUSIC_STATUS"} or hab in {"musica", "música", "playlist", "midia"}:
        return "musica"
