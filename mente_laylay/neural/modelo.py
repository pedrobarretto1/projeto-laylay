...
def adicionar_extensao_intent(
    ...
    *,
    ...
    detector = _pipeline(
        ...
        preprocessador_indicadores=preprocessador_extensao,
    ).fit(textos, rotulos)
...
def adicionar_extensao_intent_fatorada(
    ...
    *,
    ...
    for nome, representacao in sorted(representacoes.items()):
        ...
        preprocessador = (
            enriquecer_texto_estrutura_pontuacao
            if representacao == "estrutura_pontuacao"
            else enriquecer_texto_estrutura_bordas
        )
        detectores[nome] = _pipeline(
            rotulos,
            estrategia="sgd_log_loss",
            representacao=(
                "tfidf_indicadores"
                if representacao in {"estrutura_bordas", "estrutura_pontuacao"}
                else "tfidf"
            ),
