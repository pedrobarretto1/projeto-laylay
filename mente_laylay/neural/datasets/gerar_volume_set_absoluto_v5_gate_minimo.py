...
Todos os exemplos ensinam ``action=set``; somente a familia ``coloca_volume``
ensina ``intent_gate``. As probes reais com valores 50 e 100 ficam fora.
"""
...
def _item(
    ...
    *,
    ...
    return {
        ...
        "negated": False,
        "action": "set",
        "family": f"volume_set_absoluto_v5_gate_minimo_{familia}",
        "validation_group": f"volume_set_absoluto_v5_gate_minimo_{familia}",
        "validation_entity_group": "audio_volume",
        "source": "MANUAL_PARAPHRASE",
        "domain": "audio",
        "training_heads": (
            ["action", "intent_gate"]
            if familia == "coloca_volume"
            else ["action"]
        ),
...
def validar_lote(exemplos: Iterable[dict[str, Any]]) -> dict[str, Any]:
    ...
    if len(itens) != 48 or len(textos) != len(set(textos)):
        raise ValueError("lote v5_gate_minimo exige 48 textos unicos")
    if PROBES & set(textos):
        raise ValueError("probe/challenge de volume absoluto nao pode entrar no v5_gate_minimo")
    if any(_tem_valor_congelado(texto) for texto in textos):
        raise ValueError("valor congelado 50/100 nao pode entrar no lote v5_gate_minimo")
    gate_itens = [
    ...
    if len(gate_itens) != 4 or len(action_only) != 44:
        raise ValueError("lote v5_gate_minimo exige 4 exemplos de gate e 44 action-only")
    if any(
        not str(item.get("family") or "").endswith("_coloca_volume")
        for item in gate_itens
    ):
        raise ValueError("intent_gate minimo so pode usar a familia coloca_volume")
    if any(item.get("intent") != "VOLUME" for item in itens):
        raise ValueError("lote v5_gate_minimo exige intent VOLUME")
    if any(str(item.get("action") or "").casefold() != "set" for item in itens):
        raise ValueError("lote v5_gate_minimo exige action set")
    if any(item.get("is_command") is not True for item in itens):
        raise ValueError("lote v5_gate_minimo exige exemplos de comando")
    if any(item.get("negated") is not False for item in itens):
        raise ValueError("lote v5_gate_minimo nao ensina negacao")
    if any(str(item.get("domain") or "").casefold() != "audio" for item in itens):
        raise ValueError("lote v5_gate_minimo exige dominio audio")

    familias = Counter(str(item.get("family") or "") for item in itens)
    if len(familias) != 12 or set(familias.values()) != {4}:
        raise ValueError("lote v5_gate_minimo exige 12 familias com 4 niveis cada")
    ...
    if niveis != set(NIVEIS_TREINO):
        raise ValueError("niveis experimentais do lote v5_gate_minimo divergiram")
    ...
    return {
        ...
        "intent_gate_familias": ["coloca_volume"],
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=(
            ...
            "volume_set_absoluto_v5_gate_minimo.jsonl"
        ),
    )
