...
APLICATIVOS = (
    ...
    ("o Spotify", "aberto"),
    ("a galeria de fotos", "aberta"),
    ("o bloco de notas", "aberto"),
    ("a central de configurações", "aberta"),
    ("o Firefox", "aberto"),
    ("a Steam", "aberta"),
    ("o WhatsApp", "aberto"),
    ("a câmera", "aberta"),
    ...
    ("a agenda", "aberta"),
    ("o Paint", "aberto"),
    ("a planilha do Excel", "aberta"),
    ("o VLC", "aberto"),
    ("a tela de desenho", "aberta"),
    ("o PyCharm", "aberto"),
    ("a janela do Word", "aberta"),
    ("o Edge", "aberto"),
    ("a área de trabalho", "aberta"),
    ("o Teams", "aberto"),
    ("a janela do editor", "aberta"),
    ("o OBS Studio", "aberto"),
    ("a biblioteca de mídia", "aberta"),
)
...
# Congeladas antes da diversificação de entidades abaixo. São apenas validação:
# qualquer colisão textual com o lote invalida a geração.
FRASES_VALIDACAO_RESERVADAS_V2 = (
    "O navegador Brave está aberto?",
    "A ferramenta de recortes continua aberta?",
    "O editor Krita ainda está aberto?",
    "O aplicativo Fotos está rodando?",
    "A câmera do notebook está aberta?",
    "O cliente de jogos continua aberto?",
    "O painel de vídeo ainda está rodando?",
    "A central de segurança está aberta?",
    "O terminal Alacritty continua aberto?",
    "O leitor de documentos está rodando?",
)

FRASES_VALIDACAO_RESERVADAS_V3 = (
    "A interface do scanner está aberta?",
    "O monitor de recursos continua aberto?",
    "A mesa digitalizadora ainda está rodando?",
    "O cliente Matrix está aberto?",
    "A suíte de escritório continua aberta?",
    "O sincronizador de nuvem ainda está aberto?",
    "A janela auxiliar está rodando?",
    "O controlador Bluetooth continua aberto?",
    "A visualização de mapas está aberta?",
    "O editor hexadecimal ainda está rodando?",
)
...
def _exemplo(
    ...
    *,
    ...
    return {
        ...
        "validation_entity_group": validation_entity_group,
        ...
        for modalidade, molde in MOLDURAS:
            for deslocamento, alvo in enumerate(ALVOS_INVENTARIO):
                ...
                exemplos.append(_exemplo(
                    ...
                    validation_entity_group=f"alvo:{_normalizar(alvo)}",
                    extension_scope="inventario",
                ))
                indice_afirmativo += 1

    for indice_mecanismo, (mecanismo, molde) in enumerate(CONSULTAS_ALVO):
        for deslocamento in range(6):
            alvo, estado = APLICATIVOS[
                (indice_mecanismo * 6 + deslocamento) % len(APLICATIVOS)
            ]
            variante = "a" if deslocamento < 3 else "b"
            familia = f"list_windows_v1_consulta_alvo_{mecanismo}_{variante}"
            texto_consulta = molde.format(alvo=alvo, estado=estado).rstrip(" ?") + "?"
            exemplos.append(_exemplo(
                texto_consulta,
                intent="LIST_WINDOWS",
                is_command=True,
                ...
                source="MANUAL_PARAPHRASE",
                treina_negacao=indice_afirmativo % 4 == 0,
                validation_entity_group=f"entidade:{_normalizar(alvo)}",
                extension_scope="estado_alvo",
            ))
            indice_afirmativo += 1
    ...
    for indice, (mecanismo, molde) in enumerate(NEGADAS):
        ...
        for deslocamento in range(3):
            ...
            exemplos.append(_exemplo(
                ...
                treina_negacao=True,
                validation_entity_group=f"alvo:{_normalizar(alvo)}",
                extension_scope="inventario_negado",
            ))
...
def validar_lote(exemplos: Iterable[dict[str, Any]]) -> dict[str, int]:
    ...
    resumo = {
        ...
        "grupos_entidade": len({
            item["validation_entity_group"] for item in itens
        }),
