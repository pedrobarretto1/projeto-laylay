"""Onda action + intent_gate para VOLUME:set absoluto sem vazar probes.

Ensina a acao ``set`` e reforca o gate semantico de intent VOLUME. Os valores
50 e 100 e as probes reais ``coloca o volume em 50/100`` ficam fora do lote.
"""
...
def _item(
    ...
    *,
    ...
    return {
        ...
        "negated": False,
        "action": "set",
        "family": f"volume_set_absoluto_v4_{familia}",
        "validation_group": f"volume_set_absoluto_v4_{familia}",
...
def validar_lote(exemplos: Iterable[dict[str, Any]]) -> dict[str, Any]:
    ...
    if len(itens) != 48 or len(textos) != len(set(textos)):
        raise ValueError("lote v4 exige 48 textos unicos")
    if PROBES & set(textos):
        raise ValueError("probe/challenge de volume absoluto nao pode entrar no v4")
    if any(_tem_valor_congelado(texto) for texto in textos):
        raise ValueError("valor congelado 50/100 nao pode entrar no lote v4")
    if any(
        item.get("training_heads") != ["action", "intent_gate"]
        for item in itens
    ):
        raise ValueError("lote v4 so pode ensinar action + intent_gate")
    if any(item.get("intent") != "VOLUME" for item in itens):
        raise ValueError("lote v4 exige intent VOLUME")
    if any(str(item.get("action") or "").casefold() != "set" for item in itens):
        raise ValueError("lote v4 exige action set")
    if any(item.get("is_command") is not True for item in itens):
        raise ValueError("lote v4 exige exemplos de comando")
    if any(item.get("negated") is not False for item in itens):
        raise ValueError("lote v4 nao ensina negacao")
    if any(str(item.get("domain") or "").casefold() != "audio" for item in itens):
        raise ValueError("lote v4 exige dominio audio")

    familias = Counter(str(item.get("family") or "") for item in itens)
    if len(familias) != 12 or set(familias.values()) != {4}:
        raise ValueError("lote v4 exige 12 familias com 4 niveis cada")
    ...
    if niveis != set(NIVEIS_TREINO):
        raise ValueError("niveis experimentais do lote v4 divergiram")
