...
def ler_jsonl_tolerante(caminho: Path) -> tuple[list[dict[str, Any]], int]:
    """Lê um ledger append-only preservando a contagem de registros inválidos."""
...
def auditar_evidencias_shadow(
    ...
    *,
    ...
    eventos, eventos_invalidos = ler_jsonl_tolerante(
        pasta / "shadow_eventos.jsonl"
    )
    experiencias, experiencias_invalidas = ler_jsonl_tolerante(
        pasta / "experiencias.jsonl"
    )
