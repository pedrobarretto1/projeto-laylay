...
def validar_exemplo(
    ...
    *,
    ...
    grupo_entidade_bruto = exemplo.get("validation_entity_group")
    grupo_entidade = str(grupo_entidade_bruto or "").strip().casefold()
    escopo_extensao_bruto = exemplo.get("extension_scope")
    escopo_extensao = str(escopo_extensao_bruto or "").strip().casefold()
    fatores_extensao_brutos = exemplo.get("extension_factors")
    fatores_extensao: dict[str, bool] | None = None
    if fatores_extensao_brutos is not None:
        if not isinstance(fatores_extensao_brutos, Mapping):
            raise ValueError("extension_factors deve ser um mapeamento")
        fatores_extensao = {}
        for chave, valor in fatores_extensao_brutos.items():
            nome = str(chave or "").strip().casefold()
            if not nome or not isinstance(valor, bool):
                raise ValueError(
                    "extension_factors exige nomes e valores booleanos"
                )
            fatores_extensao[nome] = valor
        if not fatores_extensao:
            raise ValueError("extension_factors não pode ser vazio")
    ...
    if grupo_entidade_bruto is not None and not grupo_entidade:
        raise ValueError(
            "validation_entity_group informado não pode ser vazio"
        )
    if escopo_extensao_bruto is not None and not escopo_extensao:
        raise ValueError("extension_scope informado não pode ser vazio")
    ...
    if grupo_entidade_bruto is not None:
        validado["validation_entity_group"] = grupo_entidade
    if escopo_extensao_bruto is not None:
        validado["extension_scope"] = escopo_extensao
    if fatores_extensao is not None:
        validado["extension_factors"] = dict(sorted(fatores_extensao.items()))
