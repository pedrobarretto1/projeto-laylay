...
# 57. LAB V2.5.1 INTEGRADO EXECUTADO GREEN + SEGUNDA REVISÃO

Pedro executou o integrado após corrigir apenas wiring de harness/locks locais:

```text
falsificacao_candidato_final_turno229_LAB_V2_5_1_INTEGRADO.py
```

O primeiro run válido após os artefatos A/4.30 serem restaurados ainda foi EXIT 1 por
HEAD stale. O novo HEAD foi verificado antes do relock:

```text
53852cba7393e2addec8b52dad9ef33f68358165
```

Ele está exatamente um commit à frente do lock anterior e não alterou blobs causais de
produção. O integrado foi relockado somente depois dessa comparação.

Outro EXIT 1 ocorreu por wiring do próprio harness (`bloqueia_execucao_operacional_prioritaria`
importado do módulo errado). Depois houve um launch que alcançou FASE 3 e revelou typo de
harness `A.pendencia_delete()` vs nome real `A.pend_delete()`. Esses casos são INVALID de
harness, não REDs da hipótese A+B.

Após correção local estreita, Pedro obteve:

```text
🟢 EXIT 0 — CANDIDATO FINAL LAB V2.5.1 INTEGRADO GREEN
```

## 57.1 Evidência integrada do run

Passaram em conjunto:

```text
nascimento A receipt + B filename
fresh turn sem leak
priority FILE_SEARCH sticky bloqueado
priority live sticky bloqueado na projeção do gate
query/edit positivas preservando nao.txt
coordenador OP/RAW + filename literal
killers antes do coordenador
stale auth=True + receipt -> detector/resolver zero
CANCEL_DELETE_ITEM com filename sob sticky
CONFIRM antigo sob sticky -> zero intent
pendência antiga não engole novo CREATE_FILE
visual neutro / visual sticky
nested stale contract
side-bug move mantido separado
restauração de monkeypatches
```

Produção permaneceu intacta.

## 57.2 Segunda revisão integral — boundary de prioridade

A revisão reabriu a ordem real em `RespostaIARuntime`:
...
Fonte real em `ComandosImediatosRuntime.processar_prioritarios()` mostra:
...
Portanto o integrated LAB GREEN provou os gates auxiliares, mas não atravessou o método
inteiro de prioridade dentro de `RespostaIARuntime`.

Isso é uma lacuna de autoridade de prova real: o pre-fluxo A pode nunca ser alcançado se
a prioridade consumir o sticky primeiro.

## 57.3 Prioridade também é função mista

A mesma regra aprendida no 4.29 reaparece:

```text
sticky + consulta live        -> não consumir / não executar
sticky + capacidade estática  -> resposta segura permitida
sticky + recusa musical       -> proteção redutora permitida
sticky + cancel delete        -> prioridade devolve controle ao A
sticky + confirm delete       -> prioridade devolve; A bloqueia confirmação
```

Logo um `if sticky: return False` no método inteiro também seria amplo demais.

A prioridade precisa ser direcional por efeito.

## 57.4 Cadeias revisitadas

`segmentar_comandos_em_cadeia()` normaliza apenas uma cópia para encontrar fronteiras,
mas devolve segmentos do texto original bruto para preservar nomes, URLs, aspas e
extensões.

E `ComandosImediatosRuntime` chama:

```text
processar_cadeia(texto, "prioritario-cooperativo")
```

com o próprio texto recebido pela prioridade. Para cadeias positivas sem revisão, o RAW
permanece disponível; não foi encontrado novo root de literalidade nessa fronteira.

`processar_cadeia()` ainda merece regressão futura quando houver patch real, mas não é o
FIRST boundary aberto da segunda revisão.

## 57.5 TESTE 4.32 criado

Artefato:

```text
pos_green_v2_5_1_integrado_prioridade_direcional_teste4_32.py
```

SHA-256:

```text
b262f0992f6ec83efbf33880f6eaffb4bfc30d5a42eee6823340321d93ac8a72
```
...
```text
RespostaIARuntime REAL
 -> ComandosImediatosRuntime.processar_prioritarios REAL
 -> boundary candidato direcional
 -> pre-fluxo A REAL quando prioridade devolve controle
```

Primeiro exige reproduzir o bypass atual:

```text
quais programas estao abertos nao fecha o opera
 -> sticky=True
 -> prioridade REAL responde live
 -> pré-fluxo A nem é chamado
```

Depois testa política direcional:

```text
sticky live           -> prioridade não consome
live neutro           -> continua funcionando
capacidade sticky     -> resposta estática segura
recusa musical sticky -> bloqueio/proteção segura
cancel delete sticky  -> A recebe CANCEL_DELETE_ITEM
confirm sticky        -> A não produz CONFIRM_DELETE_ITEM
```

Um sink conversacional in-memory encerra casos não tratados para impedir LLM real.
Nenhum executor físico é fornecido.

## 57.6 Estado

```text
EIXO A .............................. FORMALMENTE FECHADO até 4.30
EIXO B .............................. FORMALMENTE FECHADO até 4.31
LAB A+B integrado ................... GREEN
segunda revisão integrada .......... PASS com boundary prioridade aberto
4.32 estático ...................... PASS
4.32 runtime ....................... NÃO EXECUTADO
produção ........................... INTACTA
patch produção ..................... BLOQUEADO até 4.32
```

Próximo passo correto: executar 4.32 exato. EXIT 0 fecha a fronteira prioridade->A da
segunda revisão; EXIT 1 é INVALID; EXIT 2 tem FIRST RED soberano.


---

# 58. REV4.1 GREEN REPORTADO + SEGUNDA REVISÃO GLOBAL-QUOTE

Data local: 21/08/2026.

> Esta seção avança SOMENTE a família B/filename/quotes. Estados anteriores não
> explicitamente reabertos permanecem históricos; não inferir daqui resultado de
> testes intermediários que não estejam registrados no corpo anterior.

Pedro executou:

```text
falsificacao_candidato_v2_5_1B_literalidade_filename_LAB_REV4_1_MERGE_FAILCLOSED.py
```

HEAD observado:

```text
5cd3582562291a947464c3bcdca3bc7b83e036d8
```

Resultado reportado:

```text
🟢 EXIT 0 — CANDIDATO LAB V2.5.1-B REV4.1 QUOTE-SAFE GREEN
```

O run confirmou:

```text
4.33 reproduzido no REV3 .................... PASS
helper quote-safe ........................... PASS na matriz declarada
malformed local fail-closed ................. PASS
smart quote E2E ............................. PASS
merge defesa em profundidade ................ PASS
executar_fluxo_intencao real / recorder ..... PASS
baseline restaurado ......................... PASS
```

SHA-256 materializado da REV4.1 exata usada na revisão:

```text
19632b091934ff1392e83f7b2f4e62bd50b778ef38bfdb7be6426e2f5ab5e4fe
```

## 58.1 Segunda revisão integral da REV4.1

O helper candidato:

```text
marcador_atom_rev4(raw,start,end,regex)
```

faz:

```text
1. prova slot arquivo/documento;
2. captura no máximo uma quote imediatamente antes de nao.<ext>;
3. se existe opener local, exige o closer esperado imediatamente depois do átomo;
4. se não existe opener local, rejeita closer órfão imediatamente depois do átomo.
```

Mas não prova:

```text
- quote órfã anterior ao slot local;
- quote extra depois do closer local;
- quote residual depois de um ato posterior;
- balanço global/coerência da região inteira que sustenta a literalidade.
```

Assim um par local válido pode coexistir com uma terceira quote órfã.

## 58.2 Falsificação direta do helper exato

A segunda revisão materializou a REV4.1 e o REV3 exatos e chamou diretamente o
helper com o regex canônico de extensões.

Observado:

```text
(True, 'nao.txt')  cria arquivo chamado "nao.txt""
(True, 'nao.txt')  cria arquivo chamado 'nao.txt''
(True, 'nao.txt')  cria arquivo chamado “nao.txt””
(True, 'nao.txt')  cria "arquivo chamado "nao.txt"
(True, 'nao.txt')  cria “arquivo chamado “nao.txt”
(True, 'nao.txt')  cria arquivo chamado "nao.txt" e fecha o opera"
(True, 'nao.txt')  cria arquivo chamado “nao.txt” e fecha o opera”
(True, 'nao.txt')  cria arquivo chamado "nao.txt" "
```

Isto falsifica a afirmação ampla “quote apertada/balanceada” quando interpretada
como estrutura global da fala. O helper garante apenas o par local.
...
O roteador continua sendo apenas amplificador downstream.

## 58.3 Regra soberana reafirmada

```text
quote não balanceada nunca pode suprimir fail-closed
```

Logo:

```text
par local válido + quote órfã residual
!=
estrutura literal provada
```

Não criar patch de produção nem integração A+B sobre REV4.1 sem fechar esta família.

## 58.4 TESTE 4.34 criado

Artefato:

```text
falsificacao_v2_5_1B_quote_global_balance_teste4_34.py
```

SHA-256:

```text
faa00ae0ae6905ca6d1866077870828f89193e068d735d5d4b05070fbfae9c3e
```
...
```text
controles REV4.1 balanceados -> continuam autorizados
quote extra depois do par    -> marker False / fail-closed
quote órfã antes do slot      -> marker False / fail-closed
quote residual após ato       -> marker False / fail-closed
```

Depois atravessa a construção real de turno da REV4.1 e, somente se houver turno
indevidamente autorizado, sonda `resolver_rev4()` SEM executar efeito físico.

Saídas:

```text
EXIT 0 = REV4.1 já satisfaz global quote balance
EXIT 1 = INVALID / lock / wiring / premissa
EXIT 2 = REV4.1 falsificada; FIRST RED manda
```

## 58.5 Estado

```text
REV4.1 runtime matriz declarada ........ GREEN reportado
segunda revisão REV4.1 ................. FALSIFICOU global-quote
helper local-pair ....................... insuficiente como prova global
4.34 estático .......................... PASS
4.34 runtime ........................... NÃO EXECUTADO
produção ............................... INTACTA
integração A+B / patch produção ........ BLOQUEADOS nesta família
```

Próximo passo correto: executar o 4.34 exato. Não criar REV4.2 antes do resultado
runtime do 4.34.
