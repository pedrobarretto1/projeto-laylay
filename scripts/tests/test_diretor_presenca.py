...
def test_log_da_proposta_expoe_resultado_real_sem_chamar_tudo_de_sombra() -> None:
    logs = []
    runtime = DiretorPresencaRuntime(
        contexto_getter=lambda: {
            "modo_jogo_ativo": True, "turno_ativo": False,
    ...
    resultado = runtime.considerar({
        "dominio": "jogo", "categoria": "companhia",
        "fala": "Essa paisagem ficou bonita, hein.", "confianca": 0.9,
        "momento_seguro": True, "evidencias": ["nova area visivel"],
        "chave": "area-nova-log",
    })

    assert resultado["proposta_comunicativa"]["status"] == "modelo_indisponivel"
    assert any(
        "[PRESENÇA:PROPOSTA]" in item
        and "status=modelo_indisponivel" in item
        and "agendada=False" in item
        and "emissao_confirmada=False" in item
        for item in logs
    )
    assert not any("[PRESENÇA:SOMBRA]" in item for item in logs)
