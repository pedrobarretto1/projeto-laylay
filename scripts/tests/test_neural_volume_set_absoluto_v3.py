...
from mente_laylay.neural.datasets.gerar_volume_set_absoluto_v3 import (
    NIVEIS_TREINO,
    PROBES,
    gerar_exemplos,
    validar_lote,
...
def test_v3_ensina_somente_action_volume_set_sem_vazar_probes():
    ...
    assert len({item["text"].casefold() for item in exemplos}) == 36
    ...
    assert all(item["training_heads"] == ["action"] for item in exemplos)
    assert all(item["domain"] == "audio" for item in exemplos)
    assert not PROBES & {item["text"].casefold() for item in exemplos}


def test_v3_nao_usa_valores_congelados_50_ou_100():
    exemplos = gerar_exemplos()
...
def test_v3_rejeita_vazamento_exato_de_probe(probe):
    exemplos = gerar_exemplos()
...
def test_v3_rejeita_outro_head_ou_action():
    exemplos = gerar_exemplos()
...
def test_v3_rejeita_valor_congelado_mesmo_em_texto_novo():
    exemplos = gerar_exemplos()
    alterados = deepcopy(exemplos)
