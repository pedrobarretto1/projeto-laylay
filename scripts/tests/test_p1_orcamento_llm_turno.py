...
@pytest.mark.parametrize("motivo", ["limite_chamadas", "principal_duplicada", "reparo_duplicado", "prazo_esgotado", "turno_obsoleto"])
def test_cliente_preserva_motivo_do_orcamento_ate_conclusao_sem_transporte(motivo):
    from mente_laylay.cognicao.estado_tecnico_llm import categoria_estado_tecnico_llm, eh_estado_tecnico_llm
    from mente_laylay.personalidade.contingencia_natural import fala_contingencia_natural
    relogio = Relogio()
    atual = {"valor": True}
    orcamento = OrcamentoLLMTurnoRuntime(monotonic=relogio, max_chamadas_turno=4)
    orcamento.iniciar_turno("causa", ainda_atual_cb=lambda: atual["valor"])
    tipo = "principal"
    if motivo == "limite_chamadas":
        for _ in range(4):
            assert orcamento.autorizar_chamada(tipo_chamada="interpretacao").permitida
    elif motivo == "principal_duplicada":
        assert orcamento.autorizar_chamada().permitida
    elif motivo == "reparo_duplicado":
        assert orcamento.autorizar_chamada(tipo_chamada="reparo_json").permitida
        tipo = "reparo_comunicacao"
    elif motivo == "prazo_esgotado":
        relogio.avancar(100)
    else:
        atual["valor"] = False
    transportes = []
    cliente = ClienteLLMRuntime(endpoint_local_getter=lambda: True,
        post_chat=lambda *a, **kw: transportes.append(kw), orcamento_turno=orcamento, log=lambda _: None)
    resultado = cliente.executar(RequisicaoTransporteLLM(
        payload={"messages": [{"role": "user", "content": "como eu poderia abrir a calculadora?"}]},
        prioridade_interativa=True, tipo_chamada=tipo))
    assert resultado.rota == "orcamento_bloqueado"
    assert transportes == []
    assert not resultado.sucesso
    assert eh_estado_tecnico_llm(resultado.texto)
    assert categoria_estado_tecnico_llm(resultado.texto) == "orcamento_" + motivo
    fala = fala_contingencia_natural("como eu poderia abrir a calculadora?",
                                   motivo_falha=categoria_estado_tecnico_llm(resultado.texto))
    assert "?" not in fala
    assert "__LAYLAY" not in fala
