...
def test_falha_do_reparo_mantem_turno_aberto_sem_inventar_resposta() -> None:
    ...
    # Saída vazia da geração não prova que faltou uma peça na entrada.
    # O contrato F3 substitui o pedido genérico de contexto por uma conclusão
    # sobre a resposta que não foi produzida; mantém turno e diagnóstico.
    assert "resposta confiável" in resposta["fala"]
    assert "?" not in resposta["fala"]
    assert resposta["suprimir_fala"] is False
    assert falhas == [(
