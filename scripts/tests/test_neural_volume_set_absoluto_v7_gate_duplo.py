...
from mente_laylay.neural.datasets.gerar_volume_set_absoluto_v7_gate_duplo import (
    NIVEIS_TREINO,
    PROBES,
    gerar_exemplos,
    validar_lote,
...
def test_v7_gate_duplo_ensina_action_e_intent_gate_volume_set_sem_vazar_probes():
    ...
    assert len(gate) == 8
    assert len(action_only) == 40
    assert all(item["family"].endswith(("_coloca_volume", "_deixa_volume")) for item in gate)
    assert all(item["domain"] == "audio" for item in exemplos)
    assert not PROBES & {item["text"].casefold() for item in exemplos}


def test_v7_gate_duplo_nao_usa_valores_congelados_50_ou_100():
    exemplos = gerar_exemplos()
...
def test_v7_gate_duplo_rejeita_vazamento_exato_de_probe(probe):
    exemplos = gerar_exemplos()
...
def test_v7_gate_duplo_rejeita_outro_head_ou_action():
    exemplos = gerar_exemplos()
...
def test_v7_gate_duplo_rejeita_valor_congelado_mesmo_em_texto_novo():
    exemplos = gerar_exemplos()
    alterados = deepcopy(exemplos)
