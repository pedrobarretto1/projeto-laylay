...
def test_observacao_comunicativa_sem_efeito_nao_exige_permissao_de_execucao() -> None:
    runtime, _estado = _motor()
    runtime.ativar_perfil_seguro_padrao()

    decisao = runtime.registrar({
        "chave": "clipboard:erro:novo",
        "tipo": "presenca_contexto_util",
    ...
    assert "observacao_sem_efeito" in decisao["motivos"]


def test_bloqueio_explicito_do_dominio_continua_valendo_para_observacao() -> None:
    runtime, _estado = _motor()
    runtime.ativar_perfil_seguro_padrao()
    runtime.configurar_dominio(
        "rotina", "bloqueado", confirmacao_explicita=True,
    )

    decisao = runtime.registrar({
        "chave": "clipboard:erro:bloqueado",
        "tipo": "presenca_contexto_util",
        "origem": "observador_area_transferencia",
        "dominio": "rotina",
        "confianca": 0.92,
    ...
    assert decisao["decisao"] not in {"sugerir", "executar", "executado"}
    assert decisao["permissao"] == "bloqueado"


def test_acao_sem_permissao_continua_fechada_mesmo_se_nao_for_executavel() -> None:
    runtime, _estado = _motor()
    runtime.ativar_perfil_seguro_padrao()

    decisao = runtime.registrar({
        "chave": "rotina:arquivo:novo",
        "tipo": "presenca_contexto_util",
        "origem": "observador_rotina",
        "dominio": "rotina",
        "confianca": 0.92,
        "utilidade": 82,
        "risco": "baixo",
        "executavel": False,
        "acao_proposta": {"intent": "CREATE_FILE", "params": {}},
    })

    assert decisao["decisao"] == "bloqueado_permissao"
