...
from mente_laylay.neural.datasets.gerar_volume_set_absoluto_v5_gate_minimo import (
    NIVEIS_TREINO,
    PROBES,
    gerar_exemplos,
    validar_lote,
...
def test_v5_gate_minimo_ensina_action_e_intent_gate_volume_set_sem_vazar_probes():
    ...
    gate = [
        item for item in exemplos
        if item["training_heads"] == ["action", "intent_gate"]
    ]
    action_only = [
        item for item in exemplos
        if item["training_heads"] == ["action"]
    ]
    assert len(gate) == 4
    assert len(action_only) == 44
    assert all(item["family"].endswith("_coloca_volume") for item in gate)
    assert all(item["domain"] == "audio" for item in exemplos)
    assert not PROBES & {item["text"].casefold() for item in exemplos}


def test_v5_gate_minimo_nao_usa_valores_congelados_50_ou_100():
    exemplos = gerar_exemplos()
...
def test_v5_gate_minimo_rejeita_vazamento_exato_de_probe(probe):
    exemplos = gerar_exemplos()
...
def test_v5_gate_minimo_rejeita_outro_head_ou_action():
    exemplos = gerar_exemplos()
...
def test_v5_gate_minimo_rejeita_valor_congelado_mesmo_em_texto_novo():
    exemplos = gerar_exemplos()
    alterados = deepcopy(exemplos)
