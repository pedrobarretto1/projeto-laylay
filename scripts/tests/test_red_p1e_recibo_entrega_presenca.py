...
def test_inicio_real_da_fala_notifica_a_fonte_da_oferta() -> None:
    estado: dict[str, Any] = {}
    opcoes_voz: dict[str, Any] = {}
    inicios_fonte: list[str] = []
    ...
    def agendar(
        ...
        **opcoes: Any,
        ...
        opcoes_voz.update(opcoes)
        return True

    resposta = RespostaEventoRuntime(
        preparacao_prompt=_PromptEvento(),
        modelo_llm=_ModeloEvento(),
        agendar_fala_proativa=agendar,
        limpar_texto_fala=lambda texto: texto,
        log=lambda _texto: None,
    )
    runtime = DiretorPresencaRuntime(
    ...
    resultado = runtime.considerar({
        ...
        "fundamentada": True,
        "momento_seguro": True,
        "motivo": "mensagem de erro copiada",
        "evidencias": ["mensagem de erro copiada", "janela atual"],
        "chave": "clipboard-inicio-real",
        "ao_iniciar": lambda: inicios_fonte.append("iniciou"),
    })

    assert resultado["proposta_comunicativa"]["agendada"] is True
    assert callable(opcoes_voz.get("ao_iniciar"))
    opcoes_voz["ao_iniciar"]()
    assert inicios_fonte == ["iniciou"]
