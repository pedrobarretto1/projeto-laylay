# -*- coding: utf-8 -*-
"""Fotografia vermelha R1.1 V2 — autoridade, cadeia e navegação contextual.

Baseline obrigatória:
    9538523a22e452481aec6d0f664de08f5a49dd5c  (teste 3.2)
...
Convenção:
- ``test_guard__*`` deve PASSAR na baseline teste 3.2;
- ``test_red__*`` deve FALHAR por assert na baseline teste 3.2;
- red inesperadamente verde exige investigação; nunca ajuste automático.
...
BASELINE_HEAD = "9538523a22e452481aec6d0f664de08f5a49dd5c"
...
# ---------------------------------------------------------------------------
# RED 1 — o ato explícito deve nascer com autoridade própria.
# ---------------------------------------------------------------------------
...
def test_red__pedido_explicito_de_aba_anterior_autoriza_o_turno(texto: str) -> None:
    turno = classificar_modalidade_turno(texto)
...
def test_red__turno_composto_do_caos_nao_nasce_como_conversa_sem_autoridade() -> None:
...
# ---------------------------------------------------------------------------
# RED 2 — se a classificação falhar, o P0 ainda precisa falhar fechado.
# ---------------------------------------------------------------------------
...
def test_red__p0_bloqueia_verbo_de_navegacao_quando_turno_nao_autorizou(texto: str) -> None:
...
# ---------------------------------------------------------------------------
# RED 3 — a mesma semântica de navegador não pode virar OPEN_URL por contexto.
# ---------------------------------------------------------------------------
...
def test_red__contexto_site_preserva_switch_previous_tab_em_vez_de_reabrir_site(texto: str) -> None:
...
# ---------------------------------------------------------------------------
# RED 4 — mudar a rota/categoria do candidato nunca aumenta autoridade.
# ---------------------------------------------------------------------------
...
def test_red__arbitro_rejeita_todo_efeito_contextual_sem_autoridade_do_turno(
    tipo_candidato: str,
    intent: str,
) -> None:
...
# ---------------------------------------------------------------------------
# RED 5 — P0 é a primeira barreira, não a única autoridade de dispatch.
# ---------------------------------------------------------------------------
...
def _runtime_com_turno_forcado(
    *,
    ...
    class Estado:
        mental = {"turno_atual": _turno(autoriza=False, modalidade="conversa", turno_id=4401)}
...
def test_red__corredor_deterministico_pos_p0_nao_executa_efeito_sem_autoridade(
    monkeypatch,
) -> None:
...
def test_red__corredor_referencia_tipificada_pos_p0_nao_executa_efeito_sem_autoridade(
    monkeypatch,
) -> None:
