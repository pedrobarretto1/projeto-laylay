#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""CANDIDATO LAB V2.5.1-B REV4.1 — FILENAME QUOTE-SAFE / MERGE FAIL-CLOSED.
...
EXIT 0 = REV4.1 GREEN no LAB; ainda exige segunda revisão integral.
EXIT 1 = lock/wiring/premissa/harness inválido.
EXIT 2 = REV4.1 falsificado; FIRST RED manda.
"""
...
def reconciliar_rev4(resultado_op,resultado_raw,*,texto_op,texto_raw,normalizar,extensoes,rev3):
    ...
    for campo in rev3.CAMPOS_LITERALIDADE.get(iop,()):
        ...
        if not canon:
            ...
            # IMPORTANTE: remover o campo da cópia antes de delegar ao REV3;
            # apenas `continue` deixaria o valor malformado original em p2 e o
            # reconciliador REV3 poderia aceitá-lo pela extensão/normalização.
            p2.pop(campo, None)
            continue
        p2[campo]=canon
    raw2["params"]=p2
...
def main():
    print("CANDIDATO LAB V2.5.1-B REV4.1 — FILENAME QUOTE-SAFE / MERGE FAIL-CLOSED")
    print("="*98)
    ...
    if reds:
        print("\n🔴 EXIT 2 — CANDIDATO V2.5.1-B REV4.1 FALSIFICADO")
    ...
    print("Quote só protege filename com estrutura apertada/balanceada; malformed RAW é removido antes do merge REV3.")
