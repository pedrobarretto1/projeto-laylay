...
# ============================================================================
# RED151-A1
# ALVO EXPLÍCITO NOVO E CURTO NÃO PODE SER REJEITADO COMO LOOKUP AMBÍGUO
# ============================================================================


def test_red151_a_novo_alvo_explicito_curto_deve_aceitar_escrita(tmp_path):
    ...
    assert resultado.get("ok") is True, (
        "RED151-A: um alvo explícito de escrita chamado 'vmz' foi rejeitado. "
        f"Resultado observado: {resultado!r}"
    )

    dados = runtime.load()

    assert "vmz" in dados, (
        "RED151-A: a playlist explícita 'vmz' deveria existir após "
        "a escrita confirmada."
    )
...
def test_red151_composto_vmz_confirmada_deve_salvar_e_responder(tmp_path):
    ...
    assert "vmz" in dados, (
        "RED151-INTEGRAÇÃO/A: a confirmação chegou ao save, mas o alvo "
        "explícito curto 'vmz' não foi materializado."
    )
